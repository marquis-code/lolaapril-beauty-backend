...
export class ReportsService {
  ...
  async generateDailySalesSummary(businessId: string, date: string): Promise<ApiResponse<DailySalesSummary>> {
    try {
      const startDate = moment(date).startOf("day").toDate();
      const endDate = moment(date).endOf("day").toDate();

      const sales: any[] = await this.saleModel.find({ businessId, createdAt: { $gte: startDate, $lte: endDate }, status: "completed" }).lean();
      const appointments: any[] = await this.appointmentModel.find({ businessId, selectedDate: date }).lean();
      const newClients: any[] = await this.clientModel.find({ businessId, createdAt: { $gte: startDate, $lte: endDate } }).lean();
  ...
  async getDailySalesSummary(businessId: string, date: string): Promise<ApiResponse<DailySalesSummary>> {
    try {
  ...
  async getWeeklySalesReport(businessId: string, startDate: string, endDate: string): Promise<ApiResponse<any>> {
    try {
  ...
  async getMonthlySalesReport(businessId: string, year: number, month: number): Promise<ApiResponse<any>> {
    try {
  ...
  private aggregateTopServices(summaries: any[]): any[] {
    ...
    summaries.forEach((summary) => {
      if (summary.servicesSummary) {
        summary.servicesSummary.forEach((service: any) => {
  ...
  private aggregateTopStaff(summaries: any[]): any[] {
    ...
    summaries.forEach((summary) => {
      if (summary.staffSummary) {
        summary.staffSummary.forEach((staff: any) => {
